...
public class TadoInterfaceImpl extends TimerTask implements TadoInterface
{
    ...
    /**
     * Generic method for requesting information from the TADO rest API about a certain home
     * @param <T> Template for the value to expect
     * @param restUrl The URL to request
     * @param homeId Home ID
     * @param returnClass Class of the return value
     * @return The information from the Tado REST API
     */
    private <T> T tadoGetHomeInfo(String restUrl, int homeId, Class<T> returnClass)
    {
        TadoAccount account=accountManager.findAccount(homeId);

        if (account!=null)
        {   
            LOG.info("Requesting REST info from {}. Account {}", restUrl, account.getUsername());
            HttpHeaders headers             = new HttpHeaders();
            headers.setBearerAuth(account.getToken().getAccess_token());
            HttpEntity entity               = new HttpEntity(headers);
            ResponseEntity<T> response      = template.exchange(restUrl, HttpMethod.GET, entity, returnClass);        
     ...
     * @return The information from the Tado REST API
     */
    private <T> List<T> tadoGetHomeInfoAsList(String restUrl, int homeId, Class<T[]> returnClass)
    {
        TadoAccount account=accountManager.findAccount(homeId);
        T[] a;
    ...
    {
        LOG.info("Requesting Tado home {}", homeId);
        return tadoGetHomeInfo("https://my.tado.com/api/v2/homes/"+homeId, homeId, TadoHome.class);      
    }

    @Override
    public List<TadoZone> tadoZones(int homeId)
    {
        LOG.info("Requesting Tado zones for home {}", homeId);
        return tadoGetHomeInfoAsList("https://my.tado.com/api/v2/homes/"+homeId+"/zones", homeId, TadoZone[].class);      
    }
    
    @Override
    public TadoState tadoPresence(int homeId)
    {
        LOG.info("Requesting Tado presence for home {}", homeId);
        return tadoGetHomeInfo("https://my.tado.com/api/v2/homes/"+homeId+"/state", homeId, TadoState.class);      
    }
    
    @Override
    public TadoZoneState tadoZoneState(int homeId, int zoneId)
    {
        LOG.info("Requesting Tado zone state for home {}, zone {}", homeId, zoneId);
        return tadoGetHomeInfo("https://my.tado.com/api/v2/homes/"+homeId+"/zones/"+zoneId+"/state", homeId, TadoZoneState.class);      
    }
    ...
    {
        LOG.info("Requesting Tado overlay state for home {}, zone {}", homeId, zoneId);
        return tadoGetHomeInfo("https://my.tado.com/api/v2/homes/"+homeId+"/zones/"+zoneId+"/overlay", homeId, TadoOverlay.class);      
    }
    ...
    {
        LOG.info("Requesting Tado devices for home {}", homeId);
        return tadoGetHomeInfoAsList("https://my.tado.com/api/v1/home/"+homeId+"/devices", homeId, TadoDevice[].class);      
    }
    ...
    {
        LOG.info("Requesting Tado time tables for home {}", homeId);
        return tadoGetHomeInfoAsList("https://my.tado.com/api/v2/homes/"+homeId+"/zones/"+zoneId+"/schedule/timetables", homeId, TadoTimeTable[].class);      
    }
    
    @Override
    public TadoTimeTable  tadoActiveTimeTable(int homeId, int zoneId)
    {
        LOG.info("Requesting chosen Tado time tables for home {}, zone {}",  homeId, zoneId);
        return tadoGetHomeInfo("https://my.tado.com/api/v2/homes/"+homeId+"/zones/"+zoneId+"/schedule/activeTimetable", homeId, TadoTimeTable.class); 
    }
    ...
    public TadoTimeTable tadoSetActiveTimeTable(int homeId, int zoneId, TadoTimeTable timeTable)
    {
        ...
        if (account!=null)
        {
            LOG.info("Set active time table for user {}, home {}, zone {} to {}", account.getUsername(), homeId, zoneId, timeTable.getId());
            HttpHeaders headers             = new HttpHeaders();
            headers.setBearerAuth(account.getToken().getAccess_token());
            HttpEntity<TadoTimeTable> entity = new HttpEntity(timeTable, headers);
            ResponseEntity<TadoTimeTable> response = template.exchange("https://my.tado.com/api/v2/homes/"+homeId+
                                                                "/zones/"+zoneId+
                                                                "/schedule/activeTimetable", 
                                                                HttpMethod.PUT, entity, TadoTimeTable.class);
            return response.getBody();
        }
        else
        {
    ...
    {
        String restUrl="https://my.tado.com/api/v2/homes/"+homeId+
                       "/zones/"+zoneId+
                       "/schedule/timetables/"+timeTableId+"/blocks";
        LOG.info("Requesting Tado schedule blocks for home {}, zone {}, time table {}.", homeId, zoneId, timeTableId);
        return this.tadoGetHomeInfoAsList(restUrl, homeId, TadoScheduleBlock[].class);
    }
    ...
    public List<TadoScheduleBlock>  tadoSetScheduleBlocks(int homeId, int zoneId, int timeTableId, List<TadoScheduleBlock> blocks)
    {
        ...
        if (account!=null)
        {
            LOG.info("Set schedule for user {}, home {}, zone {}, schedule {} day {}", 
                     account.getUsername(), 
                     homeId, 
                     zoneId, 
                     timeTableId,
                     blocks.get(0).getDayType());
            HttpHeaders headers             = new HttpHeaders();
            headers.setBearerAuth(account.getToken().getAccess_token());
            HttpEntity<List<TadoScheduleBlock>> entity = new HttpEntity(blocks, headers);
            ResponseEntity<TadoScheduleBlock[]> response = template.exchange("https://my.tado.com/api/v2/homes/"+homeId+
                                                                "/zones/"+zoneId+
                                                                "/schedule/timetables/"+timeTableId+
                                                                "/blocks/"+blocks.get(0).getDayType(), 
                                                                HttpMethod.PUT, entity, TadoScheduleBlock[].class);
            TadoScheduleBlock[] returnBlocks=response.getBody();
            return Arrays.stream(returnBlocks).collect(Collectors.toList());
        }
        else
        {
            LOG.error("No account found for home {} requested by user {}", homeId, accountManager.loggedInUsername());
    ...
    {
        LOG.info("Requesting the comfort levels for home {}", homeId);
        return tadoGetHomeInfo("https://my.tado.com/api/v2/homes/"+homeId+"/airComfort", homeId, TadoAirComfort.class);      
    }

    @Override
    public TadoWeather tadoWeather(int homeId)
    {
        LOG.info("Requesting the weather for home {}", homeId);
        return tadoGetHomeInfo("https://my.tado.com/api/v2/homes/"+homeId+"/weather", homeId, TadoWeather.class);
    }
}
